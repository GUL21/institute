	...
	if (isset($act))
{
	...
	switch ($act) {
		case 'del':
			mysql_query("UPDATE employees SET visible='0' WHERE pid='$id'");
			break;
	}
}
?>
...
<body>
	...
	<div id="block-content">
		...
		<table id="table">
 			...
 			<tbody>
 				<ul class="letter">
 							...
 							<li><a href="employees_Yi.php">Ї</a></li>
		...
		<?php
			$result = mysql_query("SELECT * FROM employees, categories WHERE employees.section=categories.name AND employees.visible='1' ORDER BY PIB ASC",$link);  
	 				if (mysql_num_rows($result) > 0)
					{
 					...
 					do
 					{
 						...
 						<tr>
 							...
 							<td>'.$row["status"].'</td>
 							<td>
 								<a href="edit_employees.php?pid='.$row["pid"].'">редагувати</a>
 								<a href="employees.php?pid='.$row["pid"].'&act=del">видалити</a>
 							</td>
 						</tr>
 						';	
 					}
